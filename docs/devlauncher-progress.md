...
- **Step graph**: IDE opens → Docker Desktop → wait-for-docker-daemon (`StepKind::WaitForDocker`) → compose up → DB readiness waits → backend starts (visible terminals) → backend port waits → API docs (enabled) → frontend dev servers (parallel) → empty terminal. Failure policies default to `SkipDependents` for steps with dependents, `WarnAndContinue` for leaves — a failing step (e.g. `npm install`) only skips its own chain and must never abort the whole run or kill a still-starting `docker compose up`.
