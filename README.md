...
> **Статус проекта:** активная open-source разработка. Версия приложения в интерфейсе — `1.0.1`, версия пакета Tauri — `0.1.0`. Формат профилей V2 и основные модули стабилизируются; перед использованием в production проверьте раздел [Известные ограничения](#известные-ограничения).
...
- Готовые installers могут отсутствовать в Releases до отдельного релизного процесса.
- Linux и macOS требуют системных зависимостей Tauri и работающего package manager; конкретный дистрибутив может потребовать ручной настройки.
- Нативный терминал отслеживает PID оболочки/terminal wrapper, а не всегда внутренней команды; его вывод находится в окне терминала, не в Log viewer.
- Detached GUI-процессы имеют `tracking_quality=detached` и не дают точного статуса завершения.
- Логи ограничены по числу строк/байтам; старые строки могут быть отброшены.
- Toolchain не устанавливает manual-only инструменты (SDK, Xcode и др.).
- Docker Compose требует установленный и запущенный Docker daemon.
- AI-настройки и assistant UI пока не выполняют запросы к моделям.
- Автоматические обновления приложения, облачная синхронизация профилей и CLI-сервис находятся в roadmap.
- Схема профилей и состав каталога могут расширяться; не полагайтесь на undocumented поля.
