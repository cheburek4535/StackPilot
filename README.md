...
> [!WARNING]
> **StackPilot выходит постепенно.** На текущем этапе полностью поддерживается **Windows**; следующим будет добавлена полная поддержка **Linux**, затем — **macOS**. Все платформы доступны, но версия для Windows самая стабильная.
>
